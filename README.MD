...
arity 2 into `delivery_report_callback` (`{delivery_report_callback, fun(DeliveryStatus, Message) -> .. end}`).
...
create_consumer() ->
    ...
    ok = erlkaf:create_consumer_group(ClientId, GroupId, ?TOPICS, ClientCfg, TopicConf, ?MODULE, []).
