...
create_consumer() ->
    ...
    ok = erlkaf:create_consumer_group(<<"group_id_here">>, [<<"benchmark">>], ProducerConfig, TopicConf, ?MODULE, []).

init(Topic, Partition, Offset, Args) ->
    io:format("init topic: ~p partition: ~p offset: ~p args: ~p ~n", [Topic, Partition, Offset, Args]),
    {ok, my_state}.
